# Comet Materia

> *Equips "Comet" magic*

## Overview

Non-elemental damage that cannot be reflected, so it works against enemies and bosses that carry Reflect. Comet hits one target hard; Comet2 fires four weaker hits at random targets, which pairs poorly with All but well against a single surviving enemy.

## At a glance

| | |
| --- | --- |
| **Category** | [Magic Materia](README.md) (green) |
| **Element** | Hidden |
| **Status** | None |
| **Max level** | 3 |
| **AP to master** | 60,000 |
| **Buy price** | 20,000 gil |

## Stat changes

| Stat | Change |
| --- | --- |
| Max HP | -5% |
| Max MP | +5% |
| Strength | -2 |
| Vitality | -1 |
| Magic | +2 |
| Magic Defense | +1 |

## Levels

| Level | Spell learned | AP to reach |
| --- | --- | --- |
| 1 | Comet | 0 |
| 2 | Comet2 | 12,000 |
| 3 | MASTER | 60,000 |

## Spells

| Spell | MP | Power | Effect |
| --- | ---: | ---: | --- |
| [**Comet**](../../spells/magic/comet.md) | 70 | 80 | Major non-elemental damage. Cannot be reflected. |
| [**Comet2**](../../spells/magic/comet2.md) | 110 | 30 | Moderate non-elemental damage to random enemy target. Four attacks. Cannot be reflected. |

## How to obtain

- **Find:** Forgotten City

---

Source: [Magic Materia (Final Fantasy VII)](https://finalfantasy.fandom.com/wiki/Magic_Materia_(Final_Fantasy_VII)), [Magic (Final Fantasy VII)](https://finalfantasy.fandom.com/wiki/Magic_(Final_Fantasy_VII)) - Final Fantasy Wiki (CC-BY-SA).
