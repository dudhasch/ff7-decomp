# Shield Materia

> *Equips "Shield" magic*

## Overview

Grants no spell at level 1. At level 2 it learns Shield, a 180 MP buff that makes the target immune to every element except non-elemental damage for several turns. Unreflectable, and one of the strongest defensive effects in the game.

## At a glance

| | |
| --- | --- |
| **Category** | Magic Materia (green) |
| **Element** | Hidden |
| **Status** | None |
| **Max level** | 3 |
| **AP to master** | 100,000 |
| **Buy price** | 1 gil |

## Stat changes

| Stat | Change |
| --- | --- |
| Max HP | -10% |
| Max MP | +10% |
| Strength | -4 |
| Vitality | -2 |
| Magic | +4 |
| Magic Defense | +2 |

## Levels

| Level | Spell learned | AP to reach |
| --- | --- | --- |
| 1 | (none) | 0 |
| 2 | Shield | 10,000 |
| 3 | MASTER | 100,000 |

## Spells

| Spell | MP | Power | Effect | In-game text |
| --- | ---: | ---: | --- | --- |
| **Shield** | 180 | 0 | Grants Shield status. Cannot be reflected. | *Extreme magic defense* |

## How to obtain

- **Find:** Northern Cave. It is possible to obtain two copies through a glitch.

---

Source: [Magic Materia (Final Fantasy VII)](https://finalfantasy.fandom.com/wiki/Magic_Materia_(Final_Fantasy_VII)) and [Magic (Final Fantasy VII)](https://finalfantasy.fandom.com/wiki/Magic_(Final_Fantasy_VII)), Final Fantasy Wiki (CC-BY-SA).
