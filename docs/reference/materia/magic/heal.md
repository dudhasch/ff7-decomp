# Heal Materia

> *Equips "Heal" magic*

## Overview

Status recovery. Poisona clears Poison only; Esuna clears nearly every common negative status; Resist pre-empts transformation statuses. The jump to Resist is expensive (52,000 AP), making Heal one of the slower Materia to master.

## At a glance

| | |
| --- | --- |
| **Category** | [Magic Materia](README.md) (green) |
| **Element** | Hidden |
| **Status** | None |
| **Max level** | 4 |
| **AP to master** | 60,000 |
| **Buy price** | 1,500 gil |

## Stat changes

| Stat | Change |
| --- | --- |
| Max HP | -2% |
| Max MP | +2% |
| Strength | -1 |
| Magic | +1 |

## Levels

| Level | Spell learned | AP to reach |
| --- | --- | --- |
| 1 | Poisona | 0 |
| 2 | Esuna | 12,000 |
| 3 | Resist | 52,000 |
| 4 | MASTER | 60,000 |

## Spells

| Spell | MP | Power | Effect |
| --- | ---: | ---: | --- |
| [**Poisona**](../../spells/magic/poisona.md) | 3 | 0 | Cures Poison. |
| [**Esuna**](../../spells/magic/esuna.md) | 15 | 0 | Cures Sleep, Poison, Sadness, Fury, Confusion, Silence, Frog, Small, Slow-numb, Petrify, Berserk, Paralyzed, and Darkness. |
| [**Resist**](../../spells/magic/resist.md) | 120 | 0 | Inflicts Resist status. |

## How to obtain

- **Buy:** Kalm, Junon, Costa del Sol (Part 1), Gongaga

---

Source: [Magic Materia (Final Fantasy VII)](https://finalfantasy.fandom.com/wiki/Magic_Materia_(Final_Fantasy_VII)), [Magic (Final Fantasy VII)](https://finalfantasy.fandom.com/wiki/Magic_(Final_Fantasy_VII)) - Final Fantasy Wiki (CC-BY-SA).
