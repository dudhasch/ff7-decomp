...
World map random encounter data is stored in the "enc_w.bin" file in the world_xx.lgp archive (on the PC version). Battles are selected based on the [region](WorldMap_Module#Regions "wikilink") and [walkmap type](WorldMap_Module#Walkmap "wikilink") of the [current triangle](../WorldMap_Module#Triangle).
