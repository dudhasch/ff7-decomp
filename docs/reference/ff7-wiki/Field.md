...
Part of the PSX background data is contained in the [MIM file](Field/MIMfile "wikilink"), it is compressed with [LZS compression](LZS_format). It consists of palettes (256 color ones) and screen blocks. No data for locating the blocks on the screen is in this file. The MIM file is a truncated TIM file and contains the normal clut location height and width information. This information is directly loaded into the PSX video ram to be decoded by the field module.

### PSX BSX Format

The Field models are contained in the [BSX file](Field/BSX "wikilink"), it is compressed with [LZS compression](LZS_format "wikilink"). [FIELD.TDB](Field/FIELD.TDB) contains the textures for these models.

### PSX BCX Format

The individual characters' field models are stored in [BCX files](Field/BCX "wikilink"). Their textures are also in [FIELD.TDB](Field/FIELD.TDB "wikilink"); they are compressed with [LZS compression](LZS_format).
...
Movies in FF7 are actually triggered from the [Field Script](Field/Script). The [F8 PMVIE](Field/Script/Opcodes/F8_PMVIE "wikilink") opcode is first used to set the movie for which the [F9 MOVIE](Field/Script/Opcodes/F9_MOVIE "wikilink") opcode is used to play. It is important to remember the MOVIE ID may vary in the PS1 versionn depending on what disc you are on. The PC movies were encoded using the DUCK (goose?) format and are AVI video files.
