...
Creates a window with a given ID and placement/size parameters. Windows are used to show [dialog](Field/Script/Opcodes/40_MESSAGE "wikilink"), present [choices](48_ASK) and so on, each of which reference the window's ID to insert text. This command only initializes a window ID, but does not present itself until a dialog command is issued on it.
