...
Sets the numerical display, as found in the [WSPCL](Field/Script/Opcodes/36_WSPCL "wikilink") opcode. The number may be set with a specified value or retrieved from two 16-bit values, and the number of digits to show is specified with *C*. Unlike the other special window setting function, [STTIM](Field/Script/Opcodes/38_STTIM "wikilink"), the [WINDOW](50_WINDOW) ID must be given for this opcode.
