...
The entire file is compressed using [LZS format](LZS_format "wikilink") and is decompressed into the playstation user memory for a battle. The file consists of several section blocks, a header, compiled HRC and model information, battle animations, a texture image in [TIM format](../PSX/TIM_format), and optional weapon models. All integers short or long are little endian based. This includes some of the [animation data](animation_format) as well.
